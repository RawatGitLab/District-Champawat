...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Champawat, Uttarakhand, India (Champawat: 29.2351, 80.0576)
    const map = L.map(mapContainerRef.current, {
      center: [29.2351, 80.0576],
      zoom: 10,
  ...
  useEffect(() => {
    ...
    layers.forEach((layerConf) => {
      ...
      if (existingLayer) {
        ...
        try {
          const isHollow = layerConf.type === "polygon" || layerConf.type === "linestring";
          existingLayer.setStyle({
            color: layerConf.color,
            fillColor: "transparent",
            weight: layerConf.weight,
            opacity: layerConf.opacity,
            fillOpacity: isHollow ? 0 : (layerConf.fillOpacity * layerConf.opacity),
            fill: !isHollow,
          } as any);
        } catch (styleErr) {
  ...
  // Map Controls: Pan to Champawat bounds safely
  const handleZoomToDistrict = () => {
    ...
    // Fallback static center of Champawat
    map.setView([29.2351, 80.0576], 10, { animate: true });
  };

  return (
    <div className={`relative flex-1 flex flex-col h-full min-w-0 ${theme === "light" ? "bg-slate-100" : "bg-slate-950"}`}>
      {/* Map Element */}
      ...
      <div className={`absolute bottom-4 left-4 z-[1000] border shadow-sm px-3 py-1.5 rounded-md flex items-center gap-4 text-xs font-mono transition-colors ${
        theme === "light"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span>X (Lng): <strong ref={lngRef} className={theme === "light" ? "text-slate-800" : "text-white"}>---</strong></span>
          <span className={theme === "light" ? "text-slate-300" : "text-slate-700"}>|</span>
          <span>Y (Lat): <strong ref={latRef} className={theme === "light" ? "text-slate-800" : "text-white"}>---</strong></span>
        </div>
        <div className={`h-3 w-px ${theme === "light" ? "bg-slate-200" : "bg-slate-700"}`} />
        <div className="text-[11px] font-semibold text-slate-400">
          Zoom: <span className="text-indigo-500 font-bold">{zoomLevel}</span>
        </div>
      </div>
      ...
      <div className="absolute top-4 right-14 z-[1000] flex flex-col gap-2">
        <button
          ...
          className={`p-2.5 border rounded-lg shadow-sm font-semibold text-xs flex items-center justify-center gap-1.5 transition-all duration-150 cursor-pointer ${
            theme === "light"
              ? "bg-white hover:bg-slate-50 border-slate-200 text-slate-700 hover:text-indigo-600"
              : "bg-slate-900 hover:bg-slate-800 border-slate-700 text-slate-200 hover:text-indigo-400"
          }`}
        >
      ...
      <div className="absolute top-4 left-4 z-[1001] w-80 font-sans">
        <div className={`relative flex items-center border rounded-lg shadow-md transition-shadow duration-200 focus-within:shadow-lg focus-within:border-indigo-400 ${
          theme === "light"
          ...
          <input
            type="text"
            className={`w-full text-xs pl-9 pr-8 py-2.5 bg-transparent rounded-lg font-semibold focus:outline-none ${
              theme === "light" ? "text-slate-700 placeholder-slate-400" : "text-slate-100 placeholder-slate-500"
            }`}
          ...
          {mapSearchQuery ? (
            ...
            <span className="absolute right-3 text-[9px] font-bold text-slate-400 pointer-events-none tracking-widest font-mono select-none">GIS</span>
          )}
        </div>

        {/* Suggestion Dropdown Panel */}
        {showMapSuggestions && filteredSearchFeatures.length > 0 && (
          <div className={`absolute left-0 right-0 mt-1.5 border rounded-lg shadow-xl overflow-hidden max-h-60 overflow-y-auto z-[1002] divide-y ${
            theme === "light"
              ? "bg-white/95 border-slate-200 divide-slate-100 text-slate-700"
            ...
            {filteredSearchFeatures.map((feat) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2.5 text-xs font-medium transition-colors flex items-center justify-between gap-1 border-none bg-transparent cursor-pointer ${
                    theme === "light"
                ...
                >
                  <span className="flex items-center gap-2 truncate">
                    ...
                    <span className={`truncate font-semibold ${theme === "light" ? "text-slate-800" : "text-slate-100"}`}>{name}</span>
                  </span>
                  <span className={`text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded font-mono shrink-0 ${
                    theme === "light"
                      ? "bg-slate-100 text-slate-400"
                      : "bg-slate-800 text-slate-400"
                  }`}>
                    {layerName.replace("USN-", "").replace("Almora-", "").replace("Champawat-", "").replace("-Boundary", "")}
                  </span>
                </button>
              );
            })}
