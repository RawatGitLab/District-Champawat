...
// Simple helper to convert HSL values to a 7-character Hex code (for color input compatibility)
function hslToHex(h: number, s: number, l: number): string {
  ...
  const buildLayerConfiguration = (loadedFeatures: GisFeature[]) => {
    ...
    const configuration: LayerConfig[] = layerNames.map((name, index) => {
      ...
      if (type === "polygon") {
        ...
        weight = 2.5;
        opacity = 1.0;
        fillOpacity = 0; // Hollow/no fill for line strings
      } else {
        // Point geometries: villages, schools, etc.
      ...
      // At start, activate ONLY the district boundary layer
      const isDistrictBoundary = lowerName.includes("district");
  ...
  const updateLayerColor = (id: string, color: string) => {
    setLayers((prev) => {
      return prev.map((l) => {
        if (l.id === id) {
          // Keep polygons and line strings completely hollow, only changing boundary color
  ...
  const handleResetToExtent = () => {
    ...
    setMeasureMode("none");
    setMeasurePoints([]);
    // Reset back to initial visibility (ONLY district boundary is visible) and ensure hollow polygons
    setLayers((prev) => prev.map((l) => {
      const isDistrictBoundary = l.name.toLowerCase().includes("district");
      const isPolygon = l.type === "polygon";
      return { 
        ...l, 
        visible: isDistrictBoundary, 
        opacity: 0.95,
          ...
          <div className="absolute inset-x-0 inset-y-0 bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center z-50 p-6 select-none font-sans">
            <div className="bg-slate-800 border border-slate-700/80 p-8 rounded-2xl shadow-2xl flex flex-col items-center max-w-sm text-center">
              ...
              <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                Connecting securely to database. Downloading geographical boundaries, river streams, and villages of <span className="text-indigo-400 font-semibold">Champawat</span>...
              </p>
